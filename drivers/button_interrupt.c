...
static irqreturn_t kdt_gpio_irq_signal_handler(int irq, void *dev_id /*, struct pt_regs *regs*/)
{
    ...
    pr_info("button_irq: kdt_gpio_irq_signal_handler: interrupt triggered!!!\n");
    ...
    if (user_space_task != NULL) {
        ...
        if (send_sig_info(SIGNR, &info, user_space_task) < 0)
            pr_err("button_irq: gpio_irq_signal: Error sending signal\n");
        else
            pr_info("button_irq: signal %d sent to pid %d\n", SIGNR, user_space_task->pid);
    }
...
static ssize_t kdt_driver_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
    ...
    if (copy_to_user(buf, "t", 1)) {
        pr_err("button_irq: read: error!\n");
        return -EFAULT;
    }

    user_space_task = current;
    pr_info("button_irq: Userspace PID: %d is registered\n", user_space_task->pid);

    wait_queue_flag = 1;
    wake_up_interruptible(&wait_queue);

    pr_info("button_irq: Value of button: %d\n", gpio_get_value(KDT_GPIO_INPUT));
...
static ssize_t kdt_driver_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
    if (copy_from_user(kernel_write_buffer, buf, count)) {
        pr_err("button_irq: write: error\n");
    }

    switch (kernel_write_buffer[0]) {
    case 0:
        pr_info("button_irq: gpio %d: low\n", KDT_GPIO_OUTPUT);
        gpio_set_value(KDT_GPIO_OUTPUT, 0);
        break;
    case 1:
        pr_info("button_irq: gpio %d: high\n", KDT_GPIO_OUTPUT);
        gpio_set_value(KDT_GPIO_OUTPUT, 1);
        break;
    default:
        pr_info("button_irq: Invalid Input!\n");
        break;
    }

    pr_info("button_irq: write: done\n");
...
{
    while (1) {
        pr_info("button_irq: waiting for event\n");
        wait_event_interruptible(wait_queue, wait_queue_flag != 0);
        if (wait_queue_flag == 2) {
            pr_info("button_irq: exit\n");
            return 0;
        }
        pr_info("button_irq: wake-up!!!\n");
        wait_queue_flag = 0;
    }
    do_exit(0);
    return 0;
...
static int __init kdt_module_init(void)
{
    ...
    if (alloc_chrdev_region(&kdt_dev, 0, 1, DRIVER_NAME) < 0) {
        pr_err("button_irq: Device Nr. could not be allocated!\n");
        return -1;
    }

    pr_info("button_irq: 할당 받은 Major = %d Minor = %d \n", MAJOR(kdt_dev), MINOR(kdt_dev));

    /* device class 생성 */
    if ((kdt_class = class_create(DRIVER_CLASS)) == NULL) {
        pr_err("button_irq: Device class can not be created!\n");
        goto cerror;
    }

    /* device file 생성 */
    if (device_create(kdt_class, NULL, kdt_dev, NULL, DRIVER_NAME) == NULL) {
        pr_err("button_irq: Can not create device file!\n");
        goto device_error;
    }
    ...
    if (cdev_add(&kdt_device, kdt_dev, 1) == -1) {
        pr_err("button_irq: Registering of device to kernel failed!\n");
        goto reg_error;
    }

    if (gpio_request(KDT_GPIO_OUTPUT, "kdt-gpio-output")) {
        pr_err("button_irq: Can not allocate GPIO %d\n", KDT_GPIO_OUTPUT);
        goto reg_error;
    }

    if (gpio_direction_output(KDT_GPIO_OUTPUT, 0)) {
        pr_err("button_irq: Can not set GPIO %d to output!\n", KDT_GPIO_OUTPUT);
        goto gpio_15_error;
    }
    ...
    if (gpio_direction_input(KDT_GPIO_INPUT)) {
        pr_err("button_irq: Can not set GPIO %d to input!\n", KDT_GPIO_INPUT);
        goto gpio_14_error;
    }

    wait_thread = kthread_create(kdt_wait_kthread, NULL, "kdt wait thread");
    if (wait_thread) {
        pr_info("button_irq: Thread created successfully\n");
        wake_up_process(wait_thread);
    } else
        pr_err("button_irq: Thread creation failed\n");
