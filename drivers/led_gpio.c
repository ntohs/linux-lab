...
static ssize_t toy_driver_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
    ...
    if (!strncmp(kernel_write_buffer, "sta_join", 8)) {
        LED_LEFT_ON();
        LED_RIGHT_ON();
        pr_info("led_gpio: LED event: sta_join\n");
    } else if (!strncmp(kernel_write_buffer, "sta_leave", 9)) {
        LED_LEFT_OFF();
        LED_RIGHT_OFF();
        pr_info("led_gpio: LED event: sta_leave\n");
        ...
        LED_RIGHT_ON();
        pr_info("led_gpio: LED event: traffic\n");
    } else {
        ...
        case 0:
            ...
            LED_LEFT_OFF();
            LED_RIGHT_OFF();
            pr_info("led_gpio: set WAIT_QUEUE_WAIT!\n");
            break;
        case 1:
            wait_queue_flag = WAIT_QUEUE_KEY;
            wake_up_interruptible(&wait_queue);
            pr_info("led_gpio: set WAIT_QUEUE_KEY!\n");
            break;
        case 2:
            wait_queue_flag = WAIT_QUEUE_NEXT;
            wake_up_interruptible(&wait_queue);
            pr_info("led_gpio: set WAIT_QUEUE_NEXT!\n");
            break;
        default:
            pr_info("led_gpio: Invalid Input!\n");
            break;
        }
    }

    pr_info("led_gpio: write: done\n");
...
static int toy_simple_io_example_1(void)
{
    ...
    for (;;) {
        if (ledvalue == ON) {
            ledvalue = OFF;
            pr_info("led_gpio: LEFT LED OFF\n");
            LED_LEFT_OFF();
        } else {
            ledvalue = ON;
            pr_info("led_gpio: LEFT LED ON\n");
            LED_LEFT_ON();
        }
        wait_queue_flag = WAIT_QUEUE_WAIT;
...
static int register_gpio_output(int gpio_num)
{
    ...
    if (gpio_request(gpio_num, name)) {
        pr_err("led_gpio: Can not allocate GPIO %d\n", gpio_num);
        return -1;
    }

    if (gpio_direction_output(gpio_num, 0)) {
        pr_err("led_gpio: Can not set GPIO %d to output!\n", gpio_num);
        gpio_free(gpio_num);
        return -1;
    }
...
static int __init toy_module_init(void)
{
    ...
    if (alloc_chrdev_region(&toy_dev, 0, 1, DRIVER_NAME) < 0) {
        pr_err("led_gpio: Device Nr. could not be allocated!\n");
        return -1;
    }

    pr_info("led_gpio: 할당 받은 Major = %d Minor = %d \n", MAJOR(toy_dev), MINOR(toy_dev));

    /* device class 생성 */
    if ((toy_class = class_create(DRIVER_CLASS)) == NULL) {
        pr_err("led_gpio: Device class can not be created!\n");
        goto cerror;
    }

    /* device file 생성 */
    if (device_create(toy_class, NULL, toy_dev, NULL, DRIVER_NAME) == NULL) {
        pr_err("led_gpio: Can not create device file!\n");
        goto device_error;
    }
    ...
    if (cdev_add(&toy_device, toy_dev, 1) == -1) {
        pr_err("led_gpio: Registering of device to kernel failed!\n");
        goto reg_error;
    }
    ...
    if (gpio_direction_input(TOY_GPIO_INPUT)) {
        pr_err("led_gpio: Can not set GPIO %d to input!\n", TOY_GPIO_INPUT);
    ...
    if (wait_thread) {
        pr_info("led_gpio: Thread created successfully\n");
        wake_up_process(wait_thread);
    } else
        pr_info("led_gpio: Thread creation failed\n");
#endif
