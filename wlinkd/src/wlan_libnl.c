...
int wlan_libnl_send_and_recv(struct nl_msg *msg, nl_recvmsg_msg_cb_t valid_cb, void *arg)
{
    ...
    opt = 1;
    setsockopt(nl_socket_get_fd(g_req_sock), SOL_NETLINK, NETLINK_EXT_ACK, &opt, sizeof(opt));

    /* NETLINK_CAP_ACK를 1로 설정 — 실패해도 무시 */
    opt = 1;
    setsockopt(nl_socket_get_fd(g_req_sock), SOL_NETLINK, NETLINK_CAP_ACK, &opt, sizeof(opt));
