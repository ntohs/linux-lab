...
#define WLAN_NL80211_MAX_STAS 64

/* 전방 선언 */
static void wlan_nl80211_update_sta(const char *bssid, int rssi_dbm, unsigned long total_pkts);
static void wlan_nl80211_remove_sta(const char bssids[][MAC_ADDR_LEN], int count);
...
/* static 변수 g_nl80211_is_ap_mode의 get API */
int wlan_nl80211_is_ap_mode()
{
    return g_nl80211_is_ap_mode;
}
 ...
 * 처리 이벤트 수신만 담당 (AP 정보, STA 목록은 타이머 주기로 wlan_nl80211_refresh()가 담당)
 */
...
static int wlan_nl80211_netdev_info_handler_cb(struct nl_msg *msg, void *arg)
{
    ...
    if (tb[NL80211_ATTR_MAC])
        wlan_utils_format_bssid(nla_data(tb[NL80211_ATTR_MAC]), bssid, sizeof(bssid));
    ...
    if (gnlh->cmd == NL80211_CMD_NEW_STATION) {
        if (bssid[0] && wlan_sta_mgr_add(bssid) > 0) {
            ...
            wlan_sta_mgr_join(bssid);
        }
        return NL_SKIP;
    } else if (gnlh->cmd == NL80211_CMD_DEL_STATION) {
        if (bssid[0]) {
            log_info("STA disconnected: %s", bssid);
            wlan_sta_mgr_leave(bssid);
            wlan_sta_mgr_remove(bssid);
            wlan_sta_mgr_list_debug();
            if (wlan_sta_mgr_count() == 0) /* all leave */
                wlan_sta_mgr_led_off();
        }
        return NL_SKIP;
    }
    ...
    if (tb[NL80211_ATTR_IFNAME])
        strncpy(g_ap_info.iface, nla_get_string(tb[NL80211_ATTR_IFNAME]), sizeof(g_ap_info.iface) - 1);

    /* Channel */
    if (tb[NL80211_ATTR_WIPHY_FREQ])
        wlan_utils_format_channel(nla_get_u32(tb[NL80211_ATTR_WIPHY_FREQ]),
                                  g_ap_info.channel, sizeof(g_ap_info.channel));

    /* Bandwidth */
    if (tb[NL80211_ATTR_CHANNEL_WIDTH])
        g_ap_info.bandwidth_mhz = wlan_nl80211_width_to_mhz(nla_get_u32(tb[NL80211_ATTR_CHANNEL_WIDTH]));

    /* Mode */
    if (tb[NL80211_ATTR_IFTYPE]) {
        iftype = nla_get_u32(tb[NL80211_ATTR_IFTYPE]);
        if (iftype == NL80211_IFTYPE_AP || iftype == NL80211_IFTYPE_P2P_GO) {
            strncpy(g_ap_info.mode, "AP", sizeof(g_ap_info.mode) - 1);
            g_nl80211_is_ap_mode = 1;
        } else if (iftype == NL80211_IFTYPE_MONITOR) {
            strncpy(g_ap_info.mode, "MONITOR", sizeof(g_ap_info.mode) - 1);
            g_nl80211_is_ap_mode = 0;
        } else {
            strncpy(g_ap_info.mode, "UNKNOWN", sizeof(g_ap_info.mode) - 1);
            g_nl80211_is_ap_mode = 0;
        }
    }
...
/* from hostapd (get_link_signal)
 * station dump 응답에서 RSSI, bitrate, packet 수를 읽어 STA 목록 등록과 활동 감지를 함께 처리
 */
static int wlan_nl80211_get_link_signal_cb(struct nl_msg *msg, void *arg)
{
    ...
    wlan_utils_format_bssid(nla_data(tb[NL80211_ATTR_MAC]), bssid_text, sizeof(bssid_text));
    ...
    if (nla_parse_nested(sinfo, NL80211_STA_INFO_MAX, tb[NL80211_ATTR_STA_INFO], policy)) {
    ...
    if (sinfo[NL80211_STA_INFO_TX_BITRATE]) {
        if (nla_parse_nested(rinfo, NL80211_RATE_INFO_MAX,
            ...
            if (rinfo[NL80211_RATE_INFO_BITRATE32])
                g_ap_info.phy_rate_mbps = (int)(nla_get_u32(rinfo[NL80211_RATE_INFO_BITRATE32]) / 10U);
            else if (rinfo[NL80211_RATE_INFO_BITRATE])
                g_ap_info.phy_rate_mbps = (int)(nla_get_u16(rinfo[NL80211_RATE_INFO_BITRATE]) / 10U);
        } else {
    ...
    if (sinfo[NL80211_STA_INFO_TX_PACKETS])
        total_packets += (unsigned long)nla_get_u32(sinfo[NL80211_STA_INFO_TX_PACKETS]);
    if (sinfo[NL80211_STA_INFO_RX_PACKETS])
        total_packets += (unsigned long)nla_get_u32(sinfo[NL80211_STA_INFO_RX_PACKETS]);

update:
    g_ap_info.rssi_dbm = rssi_dbm;
    // log_debug("get_link_signal_cb: %s rssi=%d pkts=%lu", bssid_text, rssi_dbm, total_packets);
...
static void wlan_nl80211_update_sta(const char *bssid, int rssi_dbm, unsigned long total_pkts)
{
    ...
    // log_debug("update_sta: %s rssi=%d pkts=%lu→%lu", bssid, rssi_dbm, priv_pkts, total_pkts);

    if (added > 0) {
        log_info("STA connected: %s rssi=%d pkts=%lu", bssid, rssi_dbm, total_pkts);
        wlan_sta_mgr_list_debug();
        /* 새 STA 연결 - LED 켜짐 이벤트 전송 */
        wlan_sta_mgr_join(bssid);
    } else if (total_pkts > priv_pkts) {
        /* STA 트래픽 감지 - linux-lab에 push 알림 전송 */
        wlan_sta_mgr_traffic(bssid);
        log_info(
...
int wlan_nl80211_init(const char *ifname)
{
    ...
    /* 커널에 요청을 보내고 응답을 받을 때 사용 (GET_INTERFACE, GET_STATION)
     * wlan_libnl_send_and_recv()
     */
    ...
    /* 요청할 때 쓸 nl80211 드라이버의 family ID */
    ...
    /* mlme 이벤트 감시용 소켓 - 커널이 보낸 이벤트를 "받을 때" 쓰기위함 */
    ...
    nl_socket_modify_cb(g_event_sock, NL_CB_VALID, NL_CB_CUSTOM,
    ...
    /* mlme 이벤트 감시용 소켓을 poll에 등록하기 위한 fd로 가져옴 */
...
/* 로직 추가 없으면 사실상 wlan_libnl_close() 함수 wrapper */
...
void wlan_nl80211_reset_ap_info()
{
    ...
    strncpy(g_ap_info.channel, WLAN_NOT_AVAILABLE, sizeof(g_ap_info.channel) - 1);
    wlan_utils_fill_ip_address(g_ap_info.iface, &g_ap_info);
}

/* 현재 wlan 인터페이스가 AP 모드인지 확인하고, AP 라면 station dump 로 연결 목록과 패킷 증가 여부를 갱신 */
void wlan_nl80211_refresh()
{
    struct nl_msg *msg;
    struct wlan_nl80211_sta_ctx ctx;
    char station_bssids[WLAN_NL80211_MAX_STAS][MAC_ADDR_LEN] = {{0}};
    int station_count = 0;
    ...
    genlmsg_put(msg, 0, 0, g_nl80211_id, 0, NLM_F_DUMP, NL80211_CMD_GET_INTERFACE, 0);
    if (wlan_libnl_send_and_recv(msg, wlan_nl80211_netdev_info_handler_cb, NULL) < 0) {
        log_debug("AP 기본 정보 조회 실패 (인터페이스 전환 중일 수 있음)");
        return;
    }

    if (!g_nl80211_is_ap_mode) {
        wlan_nl80211_remove_sta(NULL, -1);
        return;
    }
    ...
    genlmsg_put(msg, 0, 0, g_nl80211_id, 0, NLM_F_DUMP, NL80211_CMD_GET_STATION, 0);
    nla_put_u32(msg, NL80211_ATTR_IFINDEX, g_wlan_idx);

    memset(&ctx, 0, sizeof(ctx));
    ctx.bssids = station_bssids;
    ctx.count = &station_count;
    ctx.max_count = WLAN_NL80211_MAX_STAS;

    if (wlan_libnl_send_and_recv(msg, wlan_nl80211_get_link_signal_cb, &ctx) < 0) {
    ...
    wlan_nl80211_remove_sta(station_bssids, station_count);
}
