...
### wlinkd TCP 관리 포트 (`tcp_server_thread`, port 9000)

wlinkd 데몬 내부에서 별도 스레드로 동작하며, 외부 TCP 클라이언트에 WLAN 이벤트를 실시간으로 제공합니다.

```bash
$ nc localhost 9000

status     # 현재 AP/STA 상태를 한 번 조회합니다.
monitor    # sta_join / sta_leave 이벤트를 실시간 수신합니다.
           # (아무 키 입력하면 모니터링을 종료합니다.)
exit       # 연결을 종료합니다.
```
...
|------|------|
| **포트** | 9000 (TCP) |
| **스레드 생성** | 연결마다 `pthread_create` + `pthread_detach` |
| **Producer** | `tcp_server_push_event(event, bssid)` - `ipc.c`에서 sta_join/leave 발생 시 호출 |
| **Consumer** | `client_handler` 스레드 - `g_seq` 변화로 새 이벤트 감지 |
| **동시성 제어** | `pthread_mutex` + `pthread_cond_broadcast` + `g_seq` 카운터 |
...
커널 mlme 이벤트 → g_event_sock → poll wakeup
  → NL80211_CMD_NEW_STATION  → wlan_sta_mgr_add()  + wlan_sta_mgr_join()
  → NL80211_CMD_DEL_STATION  → wlan_sta_mgr_remove() + wlan_sta_mgr_leave()
```
...
timerfd → wlan_nl80211_refresh()
  ...
  → dump에 없는 STA 제거       → wlan_sta_mgr_leave()
```
